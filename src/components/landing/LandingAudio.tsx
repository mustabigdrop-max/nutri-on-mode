 ...
 *
 * Design principles for conversion-optimised landing pages:
 *  - Very low volume ceiling (master 0.45 vs previous 0.82)
 *  - OFF by default — user opts in by clicking
 *  - Slow evolving pad texture + soft sub-pulse (not a driving beat)
 *  - No clap/snare that feels "ad-like" or distracting
 *  - Gentle build: silence → sub pulse → warm pad → melodic texture
 *  - 96 BPM feels calm and focused (vs 128 BPM which felt rushed)
 */
...
function mkReverb(ctx: AudioContext) {
  ...
  const LEN  = Math.floor(ctx.sampleRate * 2.2); // longer plate for ambient
...
/** Soft sub-kick: just the low thump, no click transient */
...
/** Warm pad with slow attack — no percussive click */
function schedPad(ctx: AudioContext, dst: AudioNode, rev: AudioNode, t: number, chord: string) {
  ...
  freqs.forEach((freq, i) => {
    [-0.004, 0.004].forEach(det => {
      ...
      g.gain.setValueAtTime(0, t);
      g.gain.linearRampToValueAtTime(0.014, t + 1.2);  // 1.2s attack = very soft
...
/** Sparse melodic ping — pentatonic, random, far apart */
...
/** Subtle hi-hat texture — very quiet, 16th-note swing feel */
...
const LandingAudio = () => {
  ...
  const pingRef   = useRef(0); // tracks next allowed ping time
  ...
  const schedule = (ctx: AudioContext, dry: AudioNode, rev: AudioNode) => {
    ...
    while (nextRef.current < ctx.currentTime + lookahead) {
      ...
      nextRef.current += BAR / 4; // advance by 1 beat
  ...
  const startEngine = () => {
    ...
    master.gain.linearRampToValueAtTime(0.42, ctx.currentTime + 6); // very slow fade in
