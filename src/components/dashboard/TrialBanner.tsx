...
const TrialBanner = ({ trialEndsAt, plan }: TrialBannerProps) => {
  ...
  // Only show for trial users (free plan with trial_ends_at set)
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            <p className="text-sm font-bold text-foreground">Seu trial expirou</p>
            <p className="text-xs text-muted-foreground">Assine agora para continuar usando todas as funcionalidades.</p>
          </div>
          <button
            onClick={() => navigate("/#plans")}
            className="px-4 py-2 rounded-lg bg-primary text-primary-foreground text-xs font-bold whitespace-nowrap"
          >
            Assinar
          </button>
        </div>
